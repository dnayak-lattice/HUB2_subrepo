...
class CamInterface:
    ...
    def generate_raw_frames(self, camera_id):
        """
        Generator that yields raw BGR numpy arrays straight from the camera.
        No JPEG encoding — the caller is responsible for any overlay drawing
        and the single final encode.
        Args:
            camera_id: Camera device ID to generate frames from

        Yields:
            numpy.ndarray: Raw BGR frame (HxWx3)
        """
        if camera_id not in self.camera_data_context:
            return
        ...
        try:
            while True:
                ...
                sleep_time = 0.2 if self.are_both_cameras_running() else 0.033
    ...
    def generate_frames(self, camera_id):
        ...
        Wraps generate_raw_frames() with JPEG encoding for callers that need
        the legacy MJPEG-chunk interface (no overlay).
        ...
        for frame in self.generate_raw_frames(camera_id):
            try:
